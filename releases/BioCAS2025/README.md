# BioCAS 2025 Challenge Test Set

This dataset contains the **test set** for the **IEEE BioCAS 2025 Grand Challenge on Respiratory Sound Compression and Event Detection**. This challenge continues with the same tracks as 2024 (Compression and Detection) while maintaining the same dataset format.

## Dataset Overview

- **Challenge Year**: 2025
- **Task Type**: Respiratory Recording Compression & Respiratory Event Detection
- **Test Set**: 1,309 recordings (WAV + JSON pairs)
...
**This release contains only the test set for 2025.** For training, please use the training data from previous releases. Each year's challenge provides a new test set while using previously released data for training.
...
1. **Patient number** (e.g., `00015105` or `_` for anonymized)
   - Unique identifier for each patient
   - Some files may use `_` prefix for anonymized patient IDs
...
**Example**: `00015105_5.3_0_p1_25271.wav` or `_5.3_0_p8_34877.wav`
- Patient: 00015105 (or anonymized with `_`)
- Age: 5.3 years
- Gender: Male (0)
- Location: Left posterior (p1) or other location (p8)
- Recording ID: 25271
